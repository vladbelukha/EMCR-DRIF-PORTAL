...
#pragma warning disable CS8604 // Possible null reference argument.
            CreateMap<ProgressReportDetails, drr_projectprogress>(MemberList.None)
                ...
                .ForMember(dest => dest.drr_communitymediaannouncements, opt => opt.MapFrom(src => src.Workplan.MediaAnnouncement.HasValue ? (int?)Enum.Parse<DRRYesNoNotApplicable>(src.Workplan.MediaAnnouncement.Value.ToString()) : null))
                .ForMember(dest => dest.drr_commentscommunitymediaannouncements, opt => opt.MapFrom(src => src.Workplan.MediaAnnouncementComment))
                ...
                .ForPath(dest => dest.Workplan.MediaAnnouncement, opt => opt.MapFrom(src => src.drr_communitymediaannouncements.HasValue ? (int?)Enum.Parse<YesNoOption>(((DRRYesNoNotApplicable)src.drr_communitymediaannouncements).ToString()) : null))
                .ForPath(dest => dest.Workplan.MediaAnnouncementDate, opt => opt.Ignore())
                .ForPath(dest => dest.Workplan.MediaAnnouncementComment, opt => opt.MapFrom(src => src.drr_commentscommunitymediaannouncements))
