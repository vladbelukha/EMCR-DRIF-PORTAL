...
namespace EMCR.DRR.Managers.Intake
{
    public class IntakeManager : IIntakeManager
    {
        ...
        public async Task<IntakeQueryResponse> Handle(DrrApplicationsQuery q)
        {
            ...
            var res = await applicationRepository.Query(new ApplicationsQuery { Id = q.Id, BusinessId = q.BusinessId, Page = page, Count = count, OrderBy = orderBy, FilterOptions = filterOptions });
